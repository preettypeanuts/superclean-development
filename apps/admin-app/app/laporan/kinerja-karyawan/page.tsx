...
export default function KinerjaKaryawanPage() {
  ...
  const handleApplyFilters = async () => {
    ...
    if (
      ...
      setLaporanFileName(
        `kinerja_${tempSelectedKaryawan}_${formatDateAPI(
          tempStartDate
      ...
      if (karyawanData?.username) {
        ...
        const url = `/report/kinerja/detail?username=${
    ...
    } else if (reportType === 'ringkasan' && tempStartDate && tempEndDate) {
      setLaporanFileName(
        `kinerja_ringkasan_${formatDateAPI(tempStartDate)}_to_${formatDateAPI(
          tempEndDate
        )}`
      );

      const url = `/report/kinerja/ringkasan?branchId=${
      ...
      await fetchPDF(url, 'pdf', false);
    } else if (reportType === 'layanan' && tempStartDate && tempEndDate) {
      setLaporanFileName(
        `kinerja_layanan_${formatDateAPI(tempStartDate)}_to_${formatDateAPI(
          tempEndDate
        )}`
      );

      const url = `/report/kinerja/layanan?branchId=${
        tempBranch === 'all' ? '' : tempBranch
      }&startDate=${formatDateAPI(tempStartDate)}&endDate=${formatDateAPI(
        tempEndDate
      )}`;
  ...
  const handleReportTypeChange = (
    value: 'ringkasan' | 'detail' | 'layanan'
  ) => {
    setReportType(value);
    handleResetFilters();
  };
  ...
  const handleExportPDF = async () => {
    if (tempStartDate && tempEndDate) {
      try {
        ...
        switch (reportType) {
          case 'ringkasan':
            url = `/report/kinerja/ringkasan?branchId=${tempBranch}&roleId=${tempRole}&type=pdf&startDate=${formatDateAPI(
              tempStartDate
            )}&endDate=${formatDateAPI(tempEndDate)}`;
            break;
          case 'layanan':
            url = `/report/kinerja/layanan?branchId=${tempBranch}&type=pdf&startDate=${formatDateAPI(
              tempStartDate
            )}&endDate=${formatDateAPI(tempEndDate)}`;
            break;
          case 'detail':
            url = `/report/kinerja/detail?username=${selectedKaryawanUsername}&type=pdf&startDate=${formatDateAPI(
              tempStartDate
            )}&endDate=${formatDateAPI(tempEndDate)}`;
            break;
          default:
  ...
  const handleExportExcel = async () => {
    if (tempStartDate && tempEndDate) {
      try {
        ...
        switch (reportType) {
          case 'ringkasan':
            url = `/report/kinerja/ringkasan?branchId=${tempBranch}&roleId=${tempRole}&type=excel&startDate=${formatDateAPI(
              tempStartDate
            )}&endDate=${formatDateAPI(tempEndDate)}`;
            break;
          case 'layanan':
            url = `/report/kinerja/layanan?branchId=${tempBranch}&type=excel&startDate=${formatDateAPI(
              tempStartDate
            )}&endDate=${formatDateAPI(tempEndDate)}`;
            break;
          case 'detail':
            url = `/report/kinerja/detail?username=${selectedKaryawanUsername}&type=excel&startDate=${formatDateAPI(
              tempStartDate
            )}&endDate=${formatDateAPI(tempEndDate)}`;
            break;
          default:
                ...
                <div className="flex items-center space-x-4 min-w-[200px]">
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Label className="w-1/2 capitalize" htmlFor="layanan">
                        Ringkasan Layanan
                      </Label>
                    </div>
                  </RadioGroup>
